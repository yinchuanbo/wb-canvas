...
class YoutubeLink {
  ...
  loadYoutube() {
    ...
    const curH = scaleY * height;
    let html = `
		  <div class="youtube__el" style="width: ${curW - 14 - 14}px;height: ${
      curH - 14 - 40
...
class SpotifyLink {
  ...
  getSpotifyData() {
    return new Promise((resolve, reject) => {
      const fetchUrl = `https://open.spotify.com/oembed?url=${
        this.url ?? ""
      }&format=json`;
      fetch(fetchUrl)
        .then((res) => res.json())
  ...
  setSpotifyDom() {
    ...
    const curH = scaleY * height;
    let html = `
		  <div class="spotify__el" style="width: ${curW - 14 - 14}px;height: ${
      curH - 14 - 40
    }px;top: ${top + 40}px;left: ${left + 14}px;">
			 ${this.frameHtml}
			</div>
...
class CodeLink {
  ...
  setCodaCom() {
    ...
    const codaTitle = new fabric.Text("Coda", {
      left: 51,
      top: 13,
      fill: "#464646",
      fontSize: 14,
    ...
    this.group = new fabric.Group([background, codaLogo, codaTitle], {
      type: "coda",
    });
    canvas.add(this.group);
    canvas.centerObject(this.group);
  ...
  setCodaDom() {
    ...
    let html = `
					<div class="coda__el" style="width: ${curW - 14 - 14}px;height: ${
      curH - 14 - 40
    }px;top: ${top + 40}px;left: ${left + 14}px;">
		<iframe src="${url}" width="${curW - 14 - 14}" height="${
      curH - 14 - 40
